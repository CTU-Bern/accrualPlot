...
```{r}
N <- 250
dat <- data.frame(date = as.Date("2020-07-01") + sample(1:100, N, prob = seq(from = 0.01, to = 0.9, length.out = 100), replace = TRUE),
...
```{r}
par(mfrow = c(1,2))
plot(acc, which = "pred", target = 500, center_legend="strip")
# plot(acc, which = "pred", target = 500, wfun = function(x) ifelse(x$Date > (max(x$Date)-30), seq(1 / nrow(x), 1, by = 1/nrow(x)), 0))
plot(acc, which = "pred", target = 500, wfun = function(x) ifelse(x$Date > (max(x$Date)-30), 1, 0))
```

For those that prefer `ggplot2` graphics, the `engine = "ggplot"` argument to `plot` can be used to return `ggplot` objects instead of base graphics plots.

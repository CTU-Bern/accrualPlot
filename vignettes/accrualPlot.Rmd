...
plot(df2, which = "abs", unit = "week",legend.list=list(x="topleft"))
...
```{r}
par(mfrow = c(1, 3))
plot(df, which = "predict", target = 300)
plot(df2, which = "predict", target = 300)
plot(df4, which = "predict", target = 300,  center_legend="strip")
```
...
p2 <- plot(df2, which = "predict", target = c(150, 100, 100, 350), engine = "ggplot2") +
  labs(col = NULL) +
  theme_classic() +

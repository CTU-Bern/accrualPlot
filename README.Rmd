...
Or with a common start date:
```{r}
site <- sample(1:3, 50, replace = TRUE)
df4 <- accrual_create_df(x, by = site, start_date = "common")
plot(df4)
```
...
If multiple sites are included in the accrual data frame, the site info is added
```{r}
plot(df3, "predict")
```

Alternatively with a targeted number of centers:
```{r}
plot(df3, "predict", targetc = 5, center_colors = heat.colors(5), center_legend = "strip")
```

Predictions for each site can be added by specifiying separate targets
```{r}
plot(df3, "predict", target = c(30,30,30,75))
```
